import { User, UserForLogin, UserProfile } from "../data";
...
export const createUser = (user: { user: User }) => {
    return async dispatch => {
        try {
            const response = await fetch(`https://symfonywp1.amando.fun/register`, {
                method: 'POST',
...
export const loginUser = (user: { user: UserForLogin }) => {
    return async dispatch => {
        try {
            const response = await fetch(`https://symfonywp1.amando.fun/api/login`, {
                method: 'POST',
                body: JSON.stringify(user),
                headers: {
                    'Content-Type': 'application/json'
